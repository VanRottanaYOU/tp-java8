...
public class Lambda_01_Test {
    ...
    private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
    	...
    	List<Person> personnesTemp = new ArrayList<Person>();
    	for (Person p : persons) {
        	if (predicate.test(p)) {
        		personnesTemp.add(p);
        	}
        }
    	return personnesTemp;   	
    }
    // end::filter[]
    ...
    public void test_filter_by_password() throws Exception {
        ...
        System.out.println(DigestUtils.sha512Hex(passwordSha512Hex));
        PersonPredicate predicate = p -> ( p.getAge()>49 && passwordSha512Hex.equals(DigestUtils.sha512Hex(p.getPassword())));
        List<Person> result = filter(personList,predicate);
